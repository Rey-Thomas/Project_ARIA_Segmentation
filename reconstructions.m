...
%Choix reconstruction
%answer=inputdlg('Choix de la reconstruction : 1. Reconstruction du build 2. Reconstruction des défauts de MEC');
answer='1';
if strcmp(answer,'1')
    filenames_computed=filenames(contains(filenames,'MeltingEnd') & contains(filenames,'Visible'));
elseif strcmp(answer,'2')
    filenames_computed=filenames(contains(filenames,'LayeringEnd') & contains(filenames,'Visible'));
else
    disp('Saisie invalide');
    return
end

%Calcul paramètres homographie
pic=[];
for u=1:length(filenames_computed)
    try
...
answer=[1497,2447,2090,3040];
col=[answer(1);answer(2)];
row=[answer(3);answer(4)];

%Déclaration mémoire
layer_numbers=str2double(erase(erase(filenames_computed,{'Visible','IR','LayeringEnd','MeltingEnd','_','.jpg','.png'}),'Layer'));
max_layer=max(layer_numbers);
try
    data=false(length(row(1):row(2)),length(col(1):col(2)),max_layer);
catch
    disp('Mémoire insuffisante pour la zone d''intérêt spécifiée');
    return
end
...
%Calcul
for j=1:length(filenames_computed)
    try
        img=imread([pathname filenames_computed{j}]);
        if isequal(size_ref,size(img))
            ...
            %name_img=strcat('Dataset/Image/Photo_recadree_tout_nicolas_matlab/',strcat(strcat(nb_build{1},'_'),filenames_computed{j}));
            name_img=strcat('D:/ARIA/Dataset/Image/Photo_recadree/',strcat(strcat(nb_build{1},'_'),filenames_computed{j}));
...
%Vérification
if ~any(data,'all')
    disp('Reconstruction vide, vérifiez les paramètres et les données');
    return
end

%Fin
disp('Reconstruction générée avec succès');
disp('Visualisation possible avec "VolumeViewer"');
